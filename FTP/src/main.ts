...
(async () => {
  ...
  // Vite respects the base path in both dev and production modes
  ...
  async function preload() {
    ...
    for (const key of assetsToLoad) {
      let url: string = ASSETS[key as keyof typeof ASSETS];
      // Remove leading slash since basePath will be prepended
